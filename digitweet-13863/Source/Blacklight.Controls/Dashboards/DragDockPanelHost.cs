...
namespace Blacklight.Controls
{
    ...
    public class DragDockPanelHost : Canvas
    {
        ...
        private Dictionary<int, DragDockPanel> GetOrderedPanels()
        {
            ...
            {
                DragDockPanel childPanel = (DragDockPanel)child;
                childPanel.PanelIndex = (Grid.GetRow(childPanel) * this.columns) + Grid.GetColumn(childPanel);
                orderedPanels.Add(
                    (Grid.GetRow(childPanel) * this.columns) + Grid.GetColumn(childPanel),
                    childPanel);
            }
        ...
        private void DragDockPanel_Maximized(object sender, EventArgs e)
        {
            ...
            foreach (UIElement child in this.Children)
            {
                ...
                {
                    panel.PanelState = PanelState.Minimized;
                }
            }
        ...
        private void DragDockPanel_DoubleClicked(object sender, EventArgs e)
        {
            DragDockPanel doubleClicked = sender as DragDockPanel;
            if (doubleClicked != null)
            {
                if (doubleClicked.PanelState == PanelState.Minimized)
                {
                    doubleClicked.PanelState = PanelState.Restored;
                    doubleClicked.PanelState = PanelState.Maximized;
                }
                else if (doubleClicked.PanelState == PanelState.Restored)
                {
                    doubleClicked.PanelState = PanelState.Restored;
                    doubleClicked.PanelState = PanelState.Maximized;
                }
                else // if (doubleClicked.PanelState == PanelState.Maximized)
                {
                    doubleClicked.PanelState = PanelState.Restored;
                }
            }
            ...
            else
            {
                ...
                foreach (UIElement child in this.Children)
                {
                    ...
                    if (panel != this.maximizedPanel)
                    {
                        ...
                        double newHeight = (this.ActualHeight / (double)(this.Children.Count - 1)) - panel.Margin.Top - panel.Margin.Bottom;
                        if (this.minimizedPosition.Equals(MinimizedPositions.Bottom) || this.minimizedPosition.Equals(MinimizedPositions.Top))
                        {
